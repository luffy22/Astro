...
class paypal_ipn
{
  ...
  function send_response()
  {
	...
	if ($this->send_time < time() - $this->timeout) {
	  $this->error_out(JText::_("Timed out waiting for a response from PayPal."). "($this->timeout seconds)" , "");
	}
      }
      fclose( $fp );
    }
  ...
  function error_out($message, $em_headers)
  {
    ...
    $message .= "\n\n". JText::_("The following data was received from PayPal:") ."\n\n";

    @reset($this->paypal_post_vars);
    while( @list($key,$value) = @each($this->paypal_post_vars)) {
      $message .= $key . ':' . " \t$value\n";
    }
    mail($this->error_email, "[$date]" . JText::_("paypal_ipn notification"), $message, $em_headers);
  }
}
